...
<div class="container px-4">
    ...
    <form action="{{ route('seller.flashsale.variant.store', $variant) }}" method="POST" enctype="multipart/form-data">
        @csrf

        <div class="card mb-3">
            <div class="card-body">
                <h5 class="fw-bold mb-3">@lang('messages.prod_info')</h5>
                <h6 class="mb-0 fw-bold text-danger">{{ $variant->product->name }}</h6>
                <small class="text-muted">{{ $variant->variant_name }}</small>
                <div class="d-flex flex-column">
                    <span>@lang('messages.stock'): {{ $variant->stock }}</span>
                    <span>@lang('messages.price'): {{ $variant->price }}</span>
                </div>
            </div>
        </div>
            ...
            <div class="card-body">
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-6 col-12">
                        ...
                        <input type="number" step="0.01" name="price" class="form-control" min="1" max="{{ $variant->price }}" value="{{ $variant->price }}">
                    </div>
                    <div class="col-md-6 col-12">
                        <label for="stock" class="form-label fw-semibold">
                            @lang('messages.stock')
                        </label>
                        <input type="number" name="stock" class="form-control" min="1" max="{{  $variant->stock }}" value="{{ $variant->stock }}">
                    </div>
                </div>
            </div>
        </div>
